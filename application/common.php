...
function base_mini($base64){
	...
	if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
		...
		$new_file_1 = $time.'.'.$type;
		if (file_put_contents($new_file.$new_file_1, base64_decode(str_replace($result[1], '', $base64_image_content)))){
			return $new_file_1;
			$image = \think\Image::open($new_file.$new_file_1);
			//将图片裁剪为300x300并保存为crop.png
			$image->thumb(300, 300)->save($new_file.$time.'_mini.'.$type);
			//返回图片名
			return $new_file_1;
//			return $time.'_mini.'.$type;
		}else{
			return 'false';
		}
	}
